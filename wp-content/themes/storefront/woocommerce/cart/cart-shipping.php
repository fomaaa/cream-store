		...
		<?php if ( $available_methods ) : ?>
				<?php foreach ( $available_methods as $method ) : ?>
					<div class="form__radio">
						<?php
						$method->cost != 0  ? $cost =  '$'. $method->cost . ' USD' : $cost = 'Free';
						$label = explode(':', $method->label);
						if (!empty($label[0])) {
							$label[0] = '<strong>' . $label[0] . '</strong>';
							$label = implode(':', $label);
						} else {
							$label = $method->label;
						}
						...
						// printf( '<span class="form__label">%3$s</span>', $index, esc_attr( sanitize_title( $method->id ) ), wc_cart_totals_shipping_method_label( $method ) ); // WPCS: XSS ok.
						echo '<span class="form__label">'. $label .'<span class="price"><strong>'. $cost .'</strong></span></span>';
