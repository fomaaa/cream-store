...
class MailChimp_Newsletter extends MailChimp_WooCommerce_Options
{
    ...
    public function applyNewsletterField($checkout)
    {
        if (!is_admin()) {
            ...
            $checkbox .= '<div class="form__field form__field--checkbox">';
                $checkbox .= '<div class="form__checkbox">';

                $checkbox .= '<input id="mailchimp_woocommerce_newsletter" type="checkbox" name="mailchimp_woocommerce_newsletter" value="1"'.($status ? ' checked="checked"' : '').'> ';
                $checkbox .= '<span class="form__label">' . __($label, 'mailchimp-woocommerce') . '</span>';
                $checkbox .= '</div>';
            $checkbox .= '</div>';
