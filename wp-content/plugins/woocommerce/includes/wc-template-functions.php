...
if ( ! function_exists( 'woocommerce_form_field' ) ) {
	...
	function woocommerce_form_field( $key, $args, $value = null ) {
		...
		switch ( $args['type'] ) {
			...
			case 'url':
			case 'tel':
				$field .= '<input type="' . esc_attr( $args['type'] ) . '" class="input input-text ' . esc_attr( implode( ' ', $args['input_class'] ) ) . '" name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" placeholder="' . esc_attr( $args['placeholder'] ) . '"  value="' . esc_attr( $value ) . '" ' . implode( ' ', $custom_attributes ) . ' />';
...
if ( ! function_exists( 'wc_dropdown_variation_attribute_model' ) ) {
	function wc_dropdown_variation_attribute_model( $args = array() ) {
			...
			if ( $product && taxonomy_exists( $attribute ) ) {
				...
				foreach ( $terms as $key => $term ) {
					if ( in_array( $term->slug, $options, true ) ) {
						...
						$html .= '<div class="typeItem"><input id="' . esc_attr( $id ) . '" class="' . esc_attr( $class ) . '" name="' . esc_attr( $name ) . '"  type="radio" '. $checked .' value="' . esc_attr( $term->slug ) . '" ><span>'. esc_html( apply_filters( 'woocommerce_variation_option_name', $term->name ) ) . '</span></div>';
					}
					$counter++;
				}
			}
...
if ( ! function_exists( 'wc_dropdown_variation_attribute_color' ) ) {
	function wc_dropdown_variation_attribute_color( $args = array() ) {
		$args = wp_parse_args( apply_filters( 'woocommerce_dropdown_variation_attribute_options_args', $args ), array(
			'options'          => false,
			'attribute'        => false,
			'product'          => false,
